...
const CATEGORIES = {
  ...
  'Arrays': ['array_index', 'array_setindex', 'array_length', 'array_negative_index', 'slice_test'],
  'Functions': ['fn_add'],
  'Data Structures': ['map_demo', 'record_demo', 'queue_demo', 'heap_demo', 'heap_kth_smallest'],
...
function main() {
  ...
  for (const file of files) {
    ...
    if (result.success) {
      console.log(`  OK: "${result.output.trim().substring(0, 50)}${result.output.length > 50 ? '...' : ''}"`);
    } else {
  ...
  examples.sort((a, b) => {
    if (a.category !== b.category) {
      ...
      const categoryOrder = ['Basics', 'Arrays', 'Functions', 'Data Structures', 'Algorithms', 'Math', 'Strings', 'JSON & Regex', 'Other'];
