...
class RustEmitter(BaseEmitter):
    ...
    def _emit_var(self, node: dict) -> str:
        """Emit a variable reference with .clone() to satisfy Rust ownership."""
        name = node.get("name", "")
        return f"{name}.clone()"
    ...
    def _emit_binary(self, node: dict) -> str:
        ...
        if op in op_map:
            ...
            return f"op_equal(&{left}, &{right})"
        elif op == "!=":
            return f"op_not_equal(&{left}, &{right})"
        elif op == "<":
            return f"op_less_than(&{left}, &{right})"
        elif op == "<=":
            return f"op_less_than_or_equal(&{left}, &{right})"
        elif op == ">":
            return f"op_greater_than(&{left}, &{right})"
        elif op == ">=":
            return f"op_greater_than_or_equal(&{left}, &{right})"
        else:
    ...
    def _emit_for(self, node: dict) -> None:
        ...
        if isinstance(iter_expr, dict) and iter_expr.get("type") == "Range":
            ...
            self.emit_line("{")
            self.indent_level += 1
            self.emit_line(f"let __from = as_int(&{from_val});")
            self.emit_line(f"let __to = as_int(&{to_val});")
            # Use a separate counter so continue doesn't skip the increment
            self.emit_line(f"let mut __i = __from;")
            self.emit_line(f"while __i {cmp_op} __to {{")
            self.indent_level += 1
            self.emit_line(f"let mut {var} = Value::Int(__i);")
            # Increment early via defer-style: save current, increment, then run body
            # We use a scope guard pattern to ensure __i increments even on continue
            self.emit_line(f"let __cur_i = __i;")
            self.emit_line(f"__i += 1;")
            for stmt in body:
                self.emit_stmt(stmt)
            self.indent_level -= 1
            self.emit_line("}")
    ...
    def _emit_switch(self, node: dict) -> None:
        ...
        for i, case in enumerate(cases):
            ...
            self.emit_line(f"{keyword} values_equal(&__switch_val, &{case_val}) {{")
            self.indent_level += 1
