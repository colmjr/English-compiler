...
This file implements Core IL v1.2 semantics validation.
...
_ALLOWED_VERSIONS = {"coreil-0.1", "coreil-0.2", "coreil-0.3", "coreil-0.4", "coreil-0.5", "coreil-1.0", "coreil-1.1", "coreil-1.2"}
...
def validate_coreil(doc: dict) -> list[dict]:
    ...
    def validate_expr(node: Any, path: str, defined: set[str]) -> None:
        ...
        if node_type == "Call":
            ...
            if not isinstance(name, str) or not name:
                ...
                if version in ("coreil-0.5", "coreil-1.0", "coreil-1.1", "coreil-1.2") and name in _DISALLOWED_HELPER_CALLS:
                    add_error(
                        f"{path}.name",
    ...
    if version not in _ALLOWED_VERSIONS:
        add_error("$.version", "version must be 'coreil-0.1', 'coreil-0.2', 'coreil-0.3', 'coreil-0.4', 'coreil-0.5', 'coreil-1.0', 'coreil-1.1', or 'coreil-1.2'")
